...
fun getGenreListFromIds(ids: List<Int>?): List<Genres> {
    if (ids == null) {
        return emptyList()
    }
    val results = mutableListOf<Genres>()
    ids.forEach { genresMap.containsKey(it) && results.add(Genres(it, genresMap[it]!!)) }
    return results
}
